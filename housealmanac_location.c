 ...
 *
 *
 * housealmanac_location.c - Retrieve the location of the house.
 *
 * SYNOPSYS:
 *
...
static ParserToken *housealmanac_location_prepare (int count) {
...
static void housealmanac_location_update (const char *provider,
                                          char *data, int length) {

   int count = echttp_json_estimate(data);
   ParserToken *tokens = housealmanac_location_prepare (count);
...
static void housealmanac_location_discovered
   ...
   if (!status) {
       echttp_submit (0, 0, housealmanac_location_discovered, origin);
       return;
   }
   ...
   housealmanac_location_update (provider, data, length);
}

static void housealmanac_location_scan
...
void housealmanac_location_background (time_t now) {
    ...
    housediscovered ("clock", 0, housealmanac_location_scan);
}
